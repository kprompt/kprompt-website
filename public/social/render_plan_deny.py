#!/usr/bin/env python3
"""Capture plan-deny.html via Playwright → MP4 / WebM / GIF."""
...
def encode(tmp: Path, frames: int) -> None:
    ...
    mp4 = OUT_DIR / "kprompt-plan-deny.mp4"
    webm = OUT_DIR / "kprompt-plan-deny.webm"
    gif = OUT_DIR / "kprompt-plan-deny.gif"
    poster = OUT_DIR / "kprompt-plan-deny-poster.jpg"
...
async def main() -> None:
    # Duration from the HTML constant
    duration = 18.0
    with tempfile.TemporaryDirectory(prefix="kprompt-plan-deny-") as tmp:
        tmp_path = Path(tmp)
