    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          "Coordinator — cross-ns handoff + optional kube probe + Shared Knowledge (ConfigMap-durable handoff ring); mutate off (full continuous blast-radius product graph still building)",
      ...
      },
      {
        ...
        items: [
          ...
          "Knowledge Graph = service + Ingress/PVC + Secret/CM name refs + impact + memory deps; topology closure (ExternalName / env-host / EndpointSlice / NetworkPolicy) committed in ADR-0023 — Secret values stay out; Simulation MVP = change preview (docs/simulation)",
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          "AI Runtime closure pack committed (ADR-0023): Learn · Autopilot product path · continuous Coordinator · topology KG · Incident→PlanResult · cluster memory — see Building next",
      ...
      {
        type: "h2",
        text: "Building next — AI Runtime closure",
      },
      {
        type: "p",
        text: "Committed product pack (ADR-0023) that makes the motto real in-cluster. Open work — not a public ship date, not silent auto-heal.",
      },
      {
        type: "ul",
        items: [
          "Closed Learn loop — apply/resolve/FP outcomes reshape the next PlanResult (evidence-not-proof caps stay)",
          "Policy-gated Autopilot apply (product path) — Helm-first policyAuto + post-apply verify + audit; default remains propose-only",
          "Incident → PlanResult bridge — durable in-cluster proposal; approve via CLI or Slack/Team without laptop re-compile",
          ...
          "Durable cluster memory — cross-ns outcome ring biases Learn safely",
        ],
      },
      {
        type: "h2",
      ...
      },
      {
        type: "ul",
        items: [
          "Slack interface (bidirectional): ask status / why / what broke in-channel from the Observe agent — read-only answers today; mutating plans still plan → approve (Slack approve buttons are part of Runtime closure RT-008)",
