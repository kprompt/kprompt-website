    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          "Incident suggest packs: OOM memory bump, CrashLoop rollback (prior RS), ImagePull patch when the prompt names a replacement tag, probe timing relax, scheduling/PVC guidance — still approve-gated",
          "Audit harden plan: approve-gated privilege-removal (privileged / allowPrivilegeEscalation) on Deployments; invented-value fixes stay guidance",
          "Cleanup delete plan: approve-gated removal of stale Jobs / superseded ReplicaSets; ConfigMaps/Secrets stay guidance-only",
      ...
      },
      {
        type: "ul",
        items: [
          "More CrashLoop / ImagePull edge recipes beyond conservative defaults",
          "audit harden for StatefulSet / DaemonSet and non-privilege fixes (runAsNonRoot, resource requests) with safe defaults",
          "Optional ConfigMap/Secret cleanup deletes behind a stricter confirmation gate",
