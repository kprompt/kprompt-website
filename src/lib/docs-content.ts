    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          "Knowledge Graph = service + Ingress/PVC + Secret/CM name refs + impact + memory deps; Secret values / external-API / UI still building; Simulation MVP = change preview (docs/simulation)",
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          "GitHub Integration MVP: --gitops opens a GitHub PR instead of cluster apply (docs/gitops-pr); Team App install + repos + pipelines A-061…A-064 shipped — CI Checks A-065+ still Building",
        ],
      },
      {
        type: "p",
      ...
      },
      {
        ...
        items: [
          ...
          "Team CI PlanResult bridges / Checks UI (A-065…A-066; A-061…A-064 App install + repos + pipeline metadata Done)",
          "AutopilotProposal → human apply bridge (re-enter plan/approve; still not silent policyAuto)",
        ],
      },
      {
        type: "h2",
      ...
      },
      {
        type: "ul",
        items: [
          "Slack interface (bidirectional): ask status / why / what broke in-channel from the Observe agent — read-only answers today; mutating plans still plan → approve (Slack approve buttons are a separate Team bridge path)",
