...
# kprompt "scale api to 3" -n staging --approve --wait`,
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: "p",
        text: "Read episodes in order: Why AI SRE → Intent Compiler → PlanResult → Safety Engine → Multi-context → Investigation Graph → AI Timeline → Cluster Memory. Earlier positioning posts (PlanResult JSON field guide, vs kubectl-ai, plan-approve) remain companions; this series goes deeper and stays chronological.",
        links: [
          {
            label: "Why AI SRE",
...
# → Report first; mutate follow-ups still need approve`,
      ...
      },
      {
        type: "ul",
        items: [
          "Shipped: intent → PlanResult → approve; explain/logs; multi-tool routes; namespace memory priors",
          "Building: deeper investigate / why / timeline / post-apply verify; Knowledge Graph topology",
          "Exploring: richer ADR/docs knowledge — never silent mutate",
        ],
      },
      {
        type: "h2",
...
# Read the namespace_memory block — then demand Events/logs before trusting RCA`,
      ...
      {
        type: "p",
        text: "Episode 9 is Knowledge Graph — service graph, blast impact, and ADR/docs memory as topology, not a second chat brain. The hub tracks the rest of the arc.",
        links: [
          {
            label: "Series hub",
            href: "/blog/building-ai-sre-in-public",
