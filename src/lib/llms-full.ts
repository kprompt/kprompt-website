...
export function buildLlmsFullText(): string {
  const sections: string[] = [
    ...
    "- Building / next: richer Namespace Agent intelligence (beyond inventory), full continuous Coordinator blast-radius product graph, Secrets/external-API Knowledge Graph + topology UI, sandbox/chaos Simulation, Team org GitHub connect-repo (ADR-0019 accepted; A-061…A-066).",
    "- Shipped Knowledge Graph: service graph + Ingress→Service + Pod→PVC + impact + memory deps (read-only; see /docs/graph).",
    ...
    "- Shipped GitHub Integration MVP: CLI --gitops PR mode + Flux/Argo status + agent gitops evidence (Team connect-repo impl still building after ADR-0019).",
