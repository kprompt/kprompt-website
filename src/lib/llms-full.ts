...
export function buildLlmsFullText(): string {
  const sections: string[] = [
    ...
    "- Building / next: deeper continuous multi-agent reasoning, continuous mesh/OTel Coordinator blast-radius, Secret-value/external-API Knowledge Graph + topology UI, sandbox/chaos Simulation, GitHub Checks annotate write-back (App JWT; A-061…A-066 viewer/ingest shipped).",
    ...
    "- Shipped GitHub Integration MVP: CLI --gitops PR mode + Flux/Argo status + agent gitops evidence (Team SCM A-061…A-066 shipped including /ci viewer; Checks annotate write-back still Building).",
