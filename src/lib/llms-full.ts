...
export function buildLlmsFullText(): string {
  const sections: string[] = [
    ...
    "- Building / next: deeper continuous multi-agent reasoning, continuous mesh/OTel Coordinator blast-radius, Secret-value/external-API Knowledge Graph + topology UI, sandbox/chaos Simulation, Team pipelines + CI Checks (A-064…A-066; A-061…A-063 App install + repos shipped).",
    ...
    "- Shipped GitHub Integration MVP: CLI --gitops PR mode + Flux/Argo status + agent gitops evidence (Team App install + repos A-061…A-063 shipped; pipelines/CI Checks A-064+ still Building).",
