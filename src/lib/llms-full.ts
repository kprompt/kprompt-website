...
export function buildLlmsFullText(): string {
  const sections: string[] = [
    ...
    "- Building / next: deeper continuous multi-agent reasoning, continuous mesh/OTel Coordinator blast-radius, Secret-value/external-API Knowledge Graph + topology UI, sandbox/chaos Simulation, Team CI Checks / PlanResult ingest (A-065…A-066; A-061…A-064 App install + repos + pipeline metadata shipped).",
    "- Shipped Team GitHub App install MVP: GET/PUT/DELETE /v1/org/github + app /integrations (manual installation_id; live JWT mint still Building).",
    "- Shipped Team connected repos: GET/POST/DELETE /v1/org/repos + bind/unbind UI (metadata only; last_webhook_at empty until A-065).",
    ...
    "- Shipped GitHub Integration MVP: CLI --gitops PR mode + Flux/Argo status + agent gitops evidence (Team App install + repos + pipelines A-061…A-064 shipped; CI Checks A-065+ still Building).",
