...
export function buildLlmsFullText(): string {
  const sections: string[] = [
    ...
    "- Building / next: deeper continuous multi-agent reasoning, continuous mesh/OTel Coordinator blast-radius, Secret-value/external-API Knowledge Graph + topology UI, sandbox/chaos Simulation, Team PR Checks PlanResult UI (A-066; A-061…A-065 SCM + CI ingest shipped).",
    ...
    "- Shipped Team CI ingest: POST /v1/webhooks/github (HMAC) + POST /v1/ci/planresults → plan artifacts + audit reported (Checks UI still Building).",
    ...
    "- Shipped GitHub Integration MVP: CLI --gitops PR mode + Flux/Argo status + agent gitops evidence (Team SCM + CI ingest A-061…A-065 shipped; Checks UI A-066 still Building).",
