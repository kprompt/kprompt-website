...
export function buildLlmsFullText(): string {
  const sections: string[] = [
    ...
    "- Building / next: deeper continuous multi-agent reasoning, continuous mesh/OTel Coordinator blast-radius, Secret-value/external-API Knowledge Graph + topology UI, sandbox/chaos Simulation, Team org repos + CI Checks (A-062…A-066; A-061 App install metadata shipped).",
    "- Shipped Team GitHub App install MVP: GET/PUT/DELETE /v1/org/github + app /integrations (manual installation_id; live JWT mint / repos / webhooks still Building).",
    ...
    "- Shipped GitHub Integration MVP: CLI --gitops PR mode + Flux/Argo status + agent gitops evidence (Team App install A-061 shipped; repos/CI Checks A-062+ still Building).",
